function PD_sliding_window_xPlt_plot(function_name, sliding_window_cell, data_labels_struct, pd_names, filename, significance, norm, varargin)
...
if ~isempty(SW_xPlt.findaxis('Window_Dim_1')) && all_dimensions(@any, cellfun(@isempty, SW_xPlt.data))
    
    SW_xPlt = SW_xPlt.packDim('Window_Dim_1');
    
    SW_xPlt = squeeze(SW_xPlt.unpackDim(2));
    
end

if isempty(SW_xPlt.findaxis('Period'))
   
    SW_xPlt = repmat(squeeze(SW_xPlt), {'baseline1', 'baseline2'}, 'Period');
...
if sliding_window_cell{1}(1) > 1000
    ...
    for f = 1:length(ds_factors)
        ...
        SW_xPlt.data = cellfun(@(x) empty_decimate(x, ds_factors(f)), SW_xPlt.data, 'UniformOutput', 0);
...
for group = 1:length(groups_plotted)
    ...
    if ~isempty(SW_group.findaxis('Window_Dim_1'))
        ...
        %% % % Comparison over windows, plotted by recording. % % %
        ...
        %% % % Comparison of mean across windows, over recordings. % % %
...
function x = empty_decimate(x, factor)
