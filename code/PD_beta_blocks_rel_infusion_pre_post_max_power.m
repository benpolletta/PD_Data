function PD_beta_blocks_rel_infusion_pre_post_max_power(subject_mat, peak_suffix, epoch_secs, pd_handle, norm, freqs, no_cycles, bands)
        ...
        for ch = 1:no_chans
            ...
            for pd = 1:no_pds
                ...
                for sec = 1:epoch_secs
                    ...
                    BP_sec(sec, fo, pd, b, ch) = nanmax(BP_selected(sec_start:sec_end));
...
for b = 1:no_bands
    ...
    save_as_pdf(b, [subject_mat(1:(end - length('_subjects.mat'))), BP_suffix, '_pct_BP_high_', num2str(epoch_secs/60), '_min_', short_band_labels{b}, pd_handle, norm, '_max_power'])
    
end

save([subject_mat(1:(end - length('_subjects.mat'))), BP_suffix, '_pct_BP_high_', num2str(epoch_secs/60), '_min_secs', pd_handle, norm, '_max_power.mat'], 'BP_sec')
