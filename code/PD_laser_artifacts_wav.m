function PD_laser_artifacts_wav(subjects_mat)
...
for fo = 1:length(folders)
    ...
    for t = 1:no_trials
        ...
        trial_laser_on = single(round((laser_on + (t - 1)*trial_length)*sampling_freq));
        
        trial_laser_off = single(round((laser_on + (t - 1)*trial_length + lasertime)*sampling_freq));
        
        trial_off = single(round((laser_on + t*trial_length)*sampling_freq));
    ...
    save_as_pdf(gcf, [subj_name, '_wav_laser_artifacts'])
