...
for ch = 1:size(PD_dec, 2)
    ...
    fig_name = sprintf('%s_chan%d_%dclusters%.1fHz_spikes_%dprom', prefix, ch, clusters(ch), ...
        sampling_freq/min_samples_apart, min_prominences(ch));
    ...
    save_as_pdf(gcf, ['00_' fig_name '_centroids']);
    % close
    
    plot_clust_v_time(IDX, locs(peak_prom_std >= min_prominences(ch)), sampling_freq, 20);
    save_as_pdf(gcf, ['00_' fig_name '_bargraph']);
    % close
...
function plot_peaks_2(fig_name, t, sampling_freq, X, Peak_data, plot_bounds)
    ...
    for e = 1:no_epochs
        ...
        if mod(e, 3) == 1
            if e < 10
                save_as_pdf(gcf,['0' num2str(e) '_' fig_name]);
            else    
                save_as_pdf(gcf,[num2str(e) '_' fig_name]);
            end
            % close
            figure
        end
