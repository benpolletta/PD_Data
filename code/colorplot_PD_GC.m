function colorplot_PD_GC(epoch_length,freq_lim)
...
for fo = 2:length(folders)
    ...
    for i = 1:3
        ...
        imagesc([],f(f_indices),All_GC_spec(:,f_indices,i)')
        
        caxis([nanmedian(nanmin(All_GC_spec(:,f_indices,i),[],2)) nanmedian(nanmax(All_GC_spec(:,f_indices,i),[],2))])
    ...
    save_as_pdf(gcf,[shuf_name(1:end-4),'_',f_lim_label,'_cplot'])
    ...
    for i = 1:4
        ...
        imagesc((1:size(All_GC_zs,1))*epoch_length,f(f_indices),All_GC_zs(:,f_indices,i)')
        
        caxis([nanmedian(nanmin(All_GC_zs(:,f_indices,i),[],2)) nanmedian(nanmax(All_GC_zs(:,f_indices,i),[],2))])
        
        ylabel('Frequency (Hz)')
        
        axis xy, colorbar
        
        title([folder,' Spectral Granger, z-Scored, ',labels{i}])
        
    end
    
    xlabel('Time (s)')
    
    save_as_pdf(gcf,[obs_name(1:end-4),'_',f_lim_label,'_thresh_cplot'])
