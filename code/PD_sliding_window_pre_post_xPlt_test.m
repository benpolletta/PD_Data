function PD_sliding_window_pre_post_xPlt_test(function_name, sliding_window_cell, data_labels_struct, filename, significance, norm_struct, varargin)
...
for group = 1:length(groups_plotted)
    ...
    if ~any(size_dim2(:) > 1)
        ...
        if ~isempty(SW_group.findaxis('Window_Dim_1'))
            ...
            SW_WindowsPacked = squeeze(SW_group.packDim('Window_Dim_1'));
            
            function_handles = {@xp_parfor,@xp_compare_2D};
            function_arguments = {{},{@ttest, significance, [], 1}};
            dimensions = {{'Recording', 'Channel'},{'Period'}};
            [p_values, test] = recursivePlot(SW_WindowsPacked,function_handles,dimensions,function_arguments);
            
            save([group_name, '_by_subject_windows_compared_p', num2str(significance),  '_test.mat'], 'p_values', 'test')
            ...
            [p_values, test] = recursivePlot(SW_RecordingsPacked,function_handles,dimensions,function_arguments);
            
            save([group_name, '_subjects_compared_p', num2str(significance), '_test.mat'], 'p_values', 'test')
