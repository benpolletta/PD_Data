function collect_spectrum(name, subject_matnames, channels, opto_flag, peak_suffix, norm_for_power, band_index, freqs, no_cycles, bands)

if opto_flag
    
    no_secs = 5; no_trials = 10; total_secs = no_secs*no_trials;
...
All_WT_sec = nan(length(band_indices{band_index}), total_secs, total_folders, no_pds, 2);
...
for s = 1:no_mats
    ...
    if opto_flag
        ...
        load([subject_matnames{s}, BP_suffix, '_pct_', short_band_labels{band_index}, '_high_',...
            num2str(total_secs/60), '_min_secs_by_STR', norm_for_power, '_spectrum.mat'])
...
if opto_flag
    ...
    save([name, BP_suffix, '_pct_', short_band_labels{band_index}, '_',...
        num2str(no_trials), 'trials', norm_for_power, '_spectrum.mat'], 'WT_sec')
else
    
    save([name, BP_suffix, '_pct_', short_band_labels{band_index}, '_high_',...
        num2str(total_secs/60), '_min_secs', norm_for_power, '_spectrum.mat'], 'WT_sec')
