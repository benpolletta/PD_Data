function SW_xPlt = PD_pre_post_spectrum_xPlt(subjects_mat_cell, filename, norm, data_labels_struct)
...
axes_info_struct.names = {'Freq. (Hz)', 'Window_Dim_1', 'Recording', 'Channel', 'Period'};
...
for s = 1:length(subjects_mat_cell)
    ...
    axes_info_struct.values{strcmp(axes_info_struct.names, 'Channel')} = subjects_mat_struct.chan_labels;
    ...
    load([subjects_mat_name, data_labels_struct.BP_suffix, data_labels_struct.peak_suffix,...
        ...
        '_min_secs', data_labels_struct.pd_handle, norm, '_spectrum.mat'])

    sw_size = size(WT_sec);
    
    sw_xPlt = xPlt;

    sw_xPlt = sw_xPlt.importData({WT_sec});
...
save([filename, pd_label,'_band', num2str(data_labels_struct.band_index),...
    data_labels_struct.BP_suffix, data_labels_struct.peak_suffix, data_labels_struct.pd_handle, norm, '_spectrum'],...
